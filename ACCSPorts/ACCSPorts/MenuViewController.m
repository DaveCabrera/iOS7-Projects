...
- (void)viewDidLoad {
    ...
    /************************************************************************************************************************
     The following plist files reside on the manta.cs.vt.edu server computer:
     
     - accSports.plist
     - accUniversityLogos.plist
     - sportNames.plist
     
     Keeping such input data files on a server computer provides excellent maintainability, but requires network connection.
     The content of each plist file on the server can be changed, upon which the app accesses the changed data instantly.
     ***********************************************************************************************************************/
    
    // Instantiate a static dictionary object and initialize it with the content of the accSports.plist file
    self.dict1_UniversityName_Dict2 = [[NSDictionary alloc] initWithContentsOfURL:
                                       [NSURL URLWithString:@"http://manta.cs.vt.edu/iOS/accSports.plist"]];
    ...
    if (self.dict1_UniversityName_Dict2 == nil) {
    ...
    self.dict_UniversityName_UniversityLogoFileName = [[NSDictionary alloc] initWithContentsOfURL:
                                                       [NSURL URLWithString:@"http://manta.cs.vt.edu/iOS/accUniversityLogos.plist"]];
    
    if (self.dict_UniversityName_UniversityLogoFileName == nil) {
        // Create and display an error message
        UIAlertView *alertMessage = [[UIAlertView alloc] initWithTitle:@"Unable to Access File!"
                                                               message:@"Possible causes: (a) No network connection, (b) Accessed file is misplaced, or (c) Server computer is down."
                                                              delegate:nil
    ...
    self.sportNames = [[NSArray alloc] initWithContentsOfURL:
                       [NSURL URLWithString:@"http://manta.cs.vt.edu/iOS/sportNames.plist"]];
    
    if (self.sportNames == nil) {
        // Create and display an error message
        UIAlertView *alertMessage = [[UIAlertView alloc] initWithTitle:@"Unable to Access File!"
                                                               message:@"Possible causes: (a) No network connection, (b) Accessed file is misplaced, or (c) Server computer is down."
                                                              delegate:nil
